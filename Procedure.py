...
class eproc(procedure):
    def __init__(self, apparatus, executor, device, method, requirements):
        procedure.__init__(self, apparatus, executor)
        self.device = device
        self.method = method
        self.requirements = requirements
        self.name = 'eproc_'+self.device+'_'+self.method
        #dictionary of entries for the form
        #'requirement':{'desc':'Describe the requirement', 'source':'apparatus' or 'direct', 'value':14 or '', 'address'= ApparatusAdress }
    def Do(self, values={}):
        self.GetRequirements(values)
        ...
        self.Report(string='start')
        self.Report()
        self.executor.execute([[{'devices':self.device, 'procedure':self.method, 'details':details}]])
        self.Report(string='end')
